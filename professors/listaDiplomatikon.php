...
$needRefresh = (
    ...
    !isset($_SESSION['__prof_email_cached']) ||
    $_SESSION['__prof_email_cached'] !== $_SESSION['email']   
);
...
if ($needRefresh) {
    try {
        ...
        if ($pid <= 0) {
            ...
            unset($_SESSION['__prof_email_cached']);
            header("Location: login.php");
            exit();
        }
        $_SESSION['professor_id']      = $pid;
        $_SESSION['__prof_email_cached'] = $_SESSION['email']; 
    } catch (Throwable $e) {
        $_SESSION['professor_id'] = 0;
        unset($_SESSION['__prof_email_cached']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'assign') {
        ...
        try {
            // Κατοχύρωση προσωρινά του θέματος στον φοιτητή 
            $sql = "
                UPDATE theses
                SET student_id = :sid
                WHERE thesis_id = :tid
...
// Ανάκτηση ελεύθερων διπλωματικών 
...
// Διπλωματικές που επιβλέπει / συμμετέχει ===
if (isset($_GET['action']) && $_GET['action'] === 'my_theses_overview') {
    ...
    $allowedStatuses = ['Υπό Ανάθεση','Ενεργή','Υπό Εξέταση','Περατωμένη','Ακυρωμένη'];
    $hasStatus = ($status !== '' && in_array($status, $allowedStatuses, true));

    // Διπλωματικές που επιβλέπει
    $sqlSup = "
        SELECT t.thesis_id, t.title, t.status
        FROM theses t
        WHERE t.supervisor_id = :pid
    ";
    $paramsSup = [':pid' => $professor_id];
    if ($hasStatus) {
        $sqlSup .= " AND t.status = :status";
        $paramsSup[':status'] = $status;
    }
    $sqlSup .= " ORDER BY COALESCE(t.start_date, '0000-00-00') DESC, t.thesis_id DESC";

    $stmt = $pdo->prepare($sqlSup);
    $stmt->execute($paramsSup);
    $supervised = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Διπλωματικές που συμμετέχει
    $sqlPart = "
        SELECT DISTINCT t.thesis_id, t.title, t.status
        FROM committees c
  ...
  WHERE COALESCE(t.supervisor_id, c.supervisor_id) = :pid
  ORDER BY COALESCE(t.start_date, '0000-00-00') DESC, t.thesis_id DESC
";

    $paramsPart = [':pid' => $professor_id];
    if ($hasStatus) {
        $sqlPart .= " AND t.status = :status";
        $paramsPart[':status'] = $status;
    }
    $sqlPart .= " ORDER BY COALESCE(t.start_date, '0000-00-00') DESC, t.thesis_id DESC";

    $stmt = $pdo->prepare($sqlPart);
    $stmt->execute($paramsPart);
...
if (isset($_GET['action']) && $_GET['action'] === 'prof_finished_stats') {
    ...
    try {
        ...
        echo json_encode(['error'=>'server','message'=>$e->Message()]);
    }
    exit;
}
...
.add-button:hover { filter: saturate(1.05); }
...
#available-theses .assign-btn{
  background: linear-gradient(180deg,#e8eef6 0%,#cfdbee 55%,#b2c9ea 100%) !important;
  border: 1px solid #8ea9cc !important;
  color: #0b2e59 !important;
  box-shadow: inset 0 1px 0 rgba(255,255,255,.65), 0 6px 16px rgba(18,61,101,.15) !important;
}

.assign-btn:hover,
#available-theses .assign-btn:hover{
...
#myStatusFilter:focus{
  outline: none;
  border-color: #4da8da;
  box-shadow: 0 0 0 3px rgba(77,168,218,.25);
}
</style>
</head>
<body>    
...
<header class="site-header">
        ...
        <!-- Φίλτρο κατάστασης για το panel -->
        <select id="myStatusFilter" onchange="fetchMyThesesOverview()" title="Φίλτρο κατάστασης">
...
function fetchTheses() {
  ...
  fetch(url, { cache: 'no-store' })
    ...
    .then(data => {
      ...
      data.forEach(t => {
        const id = Number(t.thesis_id || t.id); 
...
function fetchMyThesesOverview(){
  const statusEl = document.getElementById('myStatusFilter');
  const status = statusEl ? statusEl.value.trim() : '';

  const url = 'listaDiplomatikon.php?action=my_theses_overview'
  ...
  fetch(url, { cache:'no-store' })
    ...
    .then(data=>{
      ...
      // Προστασία
      const supervised    = (data && Array.isArray(data.supervised)) ? data.supervised : [];
      const participating = (data && Array.isArray(data.participating)) ? data.participating : [];

      // Μετρητές
      const supCount  = document.getElementById('count-supervised');
      const partCount = document.getElementById('count-participating');
      if (supCount)  supCount.textContent  = String(supervised.length);
      if (partCount) partCount.textContent = String(participating.length);

      // Απόδοση supervised
      if (supUL){
        supUL.innerHTML = '';
        if (supervised.length === 0){
          supUL.innerHTML = '<li class="my-empty">Δεν βρέθηκαν διπλωματικές για το επιλεγμένο φίλτρο.</li>';
        } else {
          supervised.forEach(row=>{
            ...
            li.innerHTML = `
              ...
              <span class="legend-pill">${row.status ? row.status : ''}</span>
            `;
      ...
      // Απόδοση participating
      if (partUL){
        partUL.innerHTML = '';
        if (participating.length === 0){
          partUL.innerHTML = '<li class="my-empty">Δεν βρέθηκαν διπλωματικές για το επιλεγμένο φίλτρο.</li>';
        } else {
...
function openThesisDetailsModal(thesisId){
  ...
  fetch(url, { cache:'no-store' })
    ...
    .then(d => {
      ...
      const dr  = d && d.draft     ? d.draft     : null; 
