...
// === Εύρεση professor_id από το login (email) ===
$stmt = $pdo->prepare("
...
if (isset($_GET['action'])) {
    ...
    // Λίστα όλων όπου είμαι επιβλέπων (Ενεργή + Υπό Εξέταση)
    ...
    // Αλλαγή κατάστασης σε "Υπό Εξέταση" (μόνο αν είμαι επιβλέπων)
    ...
    // Αποθήκευση/ενημέρωση βαθμού + exam_date στο exam_results
    if ($_GET['action'] === 'save_grade' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        try {
            ...
            // Έλεγχος: είμαι supervisor ή μέλος τριμελούς
            $chk = $pdo->prepare("
                SELECT COUNT(*) 
                FROM theses t
    ...
    if ($_GET['action'] === 'list_other_grades') {
        ...
        try {
            ...
            // set με τα 3 ids (μπορεί κάποιο να λείπει)
            $ids = [];
            foreach (['supervisor_id','member1_id','member2_id'] as $k) {
                $v = isset($row[$k]) ? (int)$row[$k] : 0;
                if ($v > 0) $ids[$v] = true;
            }
            // βγάλε τον τρέχοντα καθηγητή (λογαριασμό)
            unset($ids[$professor_id]);

            // κράτα ΜΟΝΟ έως 2 ids (αν υπάρχουν)
            ...
            // φέρε ονόματα + βαθμό (και exam_date αν υπάρχει)
            $sql = "
                SELECT 
                    p.professor_id,
    ...
    // Λίστα διπλωματικών όπου είμαι μέλος (member1 ή member2)
    ...
    // === Έλεγχος αν είναι έτοιμη η ανακοίνωση (υπάρχουν όλα τα στοιχεία εξέτασης) ===
    if ($_GET['action'] === 'announce_ready') {
        $thesis_id = (int)($_GET['thesis_id'] ?? 0);
        if ($thesis_id <= 0) { echo json_encode(['ready'=>false]); exit; }

        // Επιτρέπει μόνο στον επιβλέποντα
    ...
    // === Πρόχειρο template ανακοίνωσης (για το modal) ===
    ...
    // === Αποθήκευση/ενημέρωση κειμένου ανακοίνωσης (μόνο επιβλέπων) ===
    if ($_GET['action'] === 'save_announcement' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Βεβαιώσου ότι υπάρχει row στο examinations
    ...
    */
    ...
    // Προσθήκη νέας σημείωσης (επιβλέπων ή μέλος)
    if ($_GET['action'] === 'add_note' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // επιτρέπεται σε supervisor ή μέλος της συγκεκριμένης διπλωματικής
        $chk = $pdo->prepare("
            SELECT 1
            FROM theses t
            LEFT JOIN committees c ON c.thesis_id = t.thesis_id
...
<style>
body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;background:#f6f8fb;margin:0;color:#233;}
...
<!-- Modal: Κείμενο Ανακοίνωσης Παρουσίασης -->
