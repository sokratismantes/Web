...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'submit_draft') {
        ...
        if (!empty($draftLink)) {
    try {
        // Φέρε τα ήδη υπάρχοντα links
        $stmt = $pdo->prepare("SELECT links FROM theses WHERE thesis_id = ?");
        $stmt->execute([$thesisId]);
        $row = $stmt->fetch();

        $existingLinks = json_decode($row['links'] ?? '[]', true);
        if (!is_array($existingLinks)) {
            $existingLinks = [];
        }

        // Πρόσθεσε τον νέο σύνδεσμο
        $existingLinks[] = $draftLink;

        // Αποθήκευσέ τον πίσω ως JSON
        $stmt = $pdo->prepare("UPDATE theses SET links = ? WHERE thesis_id = ?");
        $stmt->execute([json_encode($existingLinks, JSON_UNESCAPED_UNICODE), $thesisId]);
