...
// Ανάκτηση τύπου χρήστη και user_id
...
// Prefill δεδομένων ανά τύπο χρήστη (αν υπάρχουν τα πεδία στους πίνακες)
$userData = [
    // student
...
if ($user_type === 'student') {
    // Υποθέτουμε ότι τα πεδία βρίσκονται στον πίνακα students
    ...
    // Υποθέτουμε ότι τα πεδία βρίσκονται στον πίνακα professors
    ...
    // Υποθέτουμε ότι τα πεδία βρίσκονται στον πίνακα secretaries
    $q = "SELECT phone FROM secretaries WHERE secretary_id = ?";
    if ($st = $conn->prepare($q)) {
        $st->bind_param("i", $user_id);
        if ($st->execute()) {
            $r = $st->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Διαβάζουμε με ασφάλεια τα πεδία ανά τύπο χρήστη
    ...
    //         user_id, user_type, street, number, city, postcode, mobile, landline, department, university, phone
    $stmt->bind_param(
        "issssssssss",
        $user_id,
        $user_type,
    ...
    if ($stmt->execute()) {
        ...
        $redirectPage = ($user_type === 'student')
            ? 'student_home.php'
            : (($user_type === 'professor') ? 'professor_home.php' : 'secretary_home.php');
...
<head>
    ...
    <style>
        ...
        /* Μικρότερα σε πλάτος πεδία ΜΟΝΟ στο card "Στοιχεία Επικοινωνίας" */
        .contact-card .form-grid{
        display: grid;
        grid-template-columns: repeat(2, minmax(260px, 340px)); /* <- μικρότερο πλάτος στη στήλη */
        justify-content: center;  /* κεντράρισμα των δύο πεδίων */
        gap: 12px 16px;           /* κενά μεταξύ τους */
        }

        /* το wrapper κάθε input να μην ξεχειλώνει */
        .contact-card .input-group{
        max-width: 340px;   /* <- το «μήκος» του πλαισίου */
        margin: 0 auto;     /* κέντρο μέσα στη στήλη */
        }

        /* για mobile κρατάμε full width */
        @media (max-width: 640px){
        .contact-card .form-grid{
        ...
        /* (προαιρετικό) λίγο πιο «κομπακτ» ύψος στα ίδια πεδία */
        ...
        /* Μικρότερα/κεντραρισμένα πεδία για το card "Διεύθυνση" */
        .address-card .form-grid{
        display: grid;
        grid-template-columns: repeat(2, minmax(260px, 340px)); /* δύο μικρές στήλες */
        justify-content: center;   /* κεντράρισμα */
        gap: 12px 16px;            /* αποστάσεις */
        }
        .address-card .input-group{
        max-width: 340px;          /* «μήκος» κάθε πλαισίου */
        margin: 0 auto;
        }
        /* πιο «κομπακτ» ύψος */
        ...
        /* Στοιχεία Καθηγητή – μικρότερα, κεντραρισμένα πεδία */
        .professor-card .form-grid{
        display: grid;
        grid-template-columns: repeat(2, minmax(260px, 340px)); /* 2 στενές στήλες */
        justify-content: center; /* κεντράρισμα */
        gap: 12px 16px;
        }
...
<header class="site-header">
    ...
    <div class="right">
        <nav>
            <a href="<?php echo ($user_type==='student')?'student_home.php':(($user_type==='professor')?'professor_home.php':'secretary_home.php'); ?>">Αρχική</a>
