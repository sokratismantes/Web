...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT t.title, t.description, t.final_grade, t.repository_link, 
               s.name AS student_name,
               p1.name AS sup_name, c.supervisor_id,
               p2.name AS mem1_name, c.member1_id,
               p3.name AS mem2_name, c.member2_id
        FROM theses t
    ...
    if (count($grades) < 3) die("Δεν έχουν υποβληθεί όλοι οι βαθμοί ακόμα.");
...
<head>
  ...
  <div class="right">
    <nav>
      ...
      <a href="edit_profile.php">Το Προφιλ Μου</a>
    </nav>
    ...
    <div class="grades-boxes">
        ...
        foreach ($professors as $prof) {
            ...
            echo "<div class='grade-card'>
                    <div class='grade-name tooltip-name' data-tooltip='Καθηγητής της επιτροπής'>{$prof['name']}</div>
...
if (intro) {
    ...
    }, 600); // ελαφριά καθυστέρηση μετά το φορτωμα
}
