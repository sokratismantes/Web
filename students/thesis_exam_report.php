...
try {
    ...
    // αποθήκευση συνδέσμου αποθετηρίου 
...
<head>
    ...
    <style>
        ...
        #loader .loader-inner { animation: popin 0.6s ease-out; text-align: center; }
        ...
        .grade-name { font-weight: bold; color: #333; margin-bottom: 6px; }
        ...
        .info-row .label { min-width: 150px; font-weight: bold; }
        ...
        .download-btn:hover, .btn-soft-blue:hover{
            transform: translateY(-1px);
            background: linear-gradient(180deg, var(--sb-hover-start), var(--sb-hover-end));
        }
        .download-btn:active, .btn-soft-blue:active{
            transform: translateY(0);
            background: linear-gradient(180deg, var(--sb-active-start), var(--sb-active-end));
        }
        .download-btn:focus-visible, .btn-soft-blue:focus-visible{
            outline:3px solid var(--sb-focus);
            outline-offset:2px;
        }

        .tooltip-name { position: relative; cursor: help; }
        .tooltip-name::after {
            content: attr(data-tooltip);
            position: absolute; bottom: 125%; left: 50%; transform: translateX(-50%);
            background-color: #333; color: white; padding: 6px 10px; border-radius: 5px;
            white-space: nowrap; font-size: 0.75rem; opacity: 0; pointer-events: none; transition: opacity 0.3s;
        }
        .tooltip-name:hover::after { opacity: 1; }

        .grade-score.badge {
            display: inline-block; padding: 5px 10px; border-radius: 12px; font-weight: bold; font-size: 0.95rem; color: white;
        }
        ...
        .green-grade { color: #28a745; font-weight: bold; }

        .timeline-box-wrapper { position: absolute; top: 40px; right: 50px; z-index: 100; }
        .timeline-box {
            position: fixed; top: 170px; right: 30px; background-color: #ffffff;
            border-left: 4px solid #007bff; padding: 14px 18px; border-radius: 10px;
            box-shadow: 0 6px 16px rgba(0,0,0,0.12); width: 220px; font-size: 0.88rem; color: #333; z-index: 1000;
            opacity: 0; transition: opacity 0.8s ease;
        }
        .timeline-box h5 { margin-top: 0; font-size: 1rem; margin-bottom: 10px; color: #003366; }
        .timeline-box ul { list-style-type: none; padding-left: 0; margin: 0; }
        .timeline-box li { margin-bottom: 8px; }
        @media (max-width: 768px) { .timeline-box-wrapper { display: none; } }
        ...
        .form-container.repo-card .form-group { margin-bottom: 14px; }
        .form-container.repo-card label {
            display: block; margin-bottom: 6px; font-weight: 600; color: #334155; font-size: 14px;
        }

        #repository_link, #repo_thesis_id {
            width: 100%; height: 44px; padding: 10px 12px;
            border: 1px solid #cbd5e1; border-radius: 10px; font-size: 14px; color: #1f2937; background: #fff;
            transition: border-color .2s ease, box-shadow .2s ease;
        }
        #repository_link::placeholder { color: #9aa7b5; }
        #repository_link:focus, #repo_thesis_id:focus {
        ...
        #repo_msg { font-size: 14px; min-height: 18px; text-align:center; margin-top:8px; font-weight:600; }

        footer {
            flex-shrink: 0; width: 100%;
            background-color: rgba(0, 51, 102, 0.92);
            color: white; text-align: center; padding: 30px; margin-top: 20px;
        }
    </style>
</head>
<body>
    ...
    <div style="display: flex; gap: 20px; margin-top: 25px; flex-wrap: wrap;">
        ...
        <div class="info-card">
            ...
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div id="qrcode"
                     data-repo-url="<?= htmlspecialchars($thesis['repository_link'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>">
                </div>
            </div>
        </div>
        <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!qrEl) return;
  var url = qrEl.getAttribute('data-repo-url') || '';
  if (url) {
    new QRCode(qrEl, { text: url, width: 64, height: 64 });
  }
});
</script>
