        ...
        if ($_POST['pn_action'] === 'accept') {
            try {
                ...
                if ($thesisId > 0) {
                    ...
                    /* ΝΕΟ: αν έχουν συμπληρωθεί και οι δύο θέσεις επιτροπής, ενεργοποιούμε τη διπλωματική */
                    $stmt = $pdo->prepare("
                        SELECT member1_id, member2_id
                        FROM committees
                        WHERE thesis_id = :tid
...
<head>
    ...
    <style>
        ...
        body {font-family: 'Segoe UI', Tahoma, sans-serif; 
            margin: 0; 
            padding: 0; 
            background: linear-gradient(to right, #e2e2e2, #c9d6ff); color: #333;
        }
        body::before { content: ""; 
            position: fixed; 
            inset: 0; 
        ...
        h1 {
            ...
            letter-spacing: 1px;
            animation: fadeSlideIn .5s ease-out both .05s;
        }
        h2.section-title {
            ...
            color: #004085;
            font-weight: 600;
            animation: fadeSlideIn .5s ease-out both .08s;
        }
        ...
        .table-wrapper {
            ...
            margin: 0 auto;
            max-width: 95%;
            animation: fadeSlideIn .5s ease-out both .12s; 
        }
        ...
        .button {
            ...
            animation: fadeSlideIn .45s ease-out both .18s;
        }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  rows.forEach((tr, i) => {
    tr.style.animationDelay = (i * 60) + 'ms'; // 60ms κλιμάκωση
